...
"""Poppy 环境的共享骨架：场景 / 动作 / 观测 / 事件 / 终止 / 基础奖励。

==============================================================================
这个文件里放什么、不放什么
==============================================================================
放：**站和走都完全一样的部分**。
    场景（平地 + 接触传感器）、动作空间、观测分组、随机化事件、
    终止条件、以及"保持像个人样"的基础奖励项（姿态、高度、正则、防作弊）。

不放：速度指令（站立恒 0、行走要采样）和步态奖励（只有行走需要）。
      这两样在各任务的 `*_env_cfg.py` 里 override。

==============================================================================
坐标系与单位（踩过的坑写在这里，避免重新踩）
==============================================================================
  ...
  · 关节限位（rad，D3 实测，已逐项核对镜像关系）：
        ...
        ankle_y [-0.785, +0.785]
  · 自由度语义（从 URDF 的 origin/axis 逐个推出来的，不是从名字猜的）：
        hip_x → 绕骨盆前后轴 → **侧摆/外展**
        hip_z → 绕骨盆竖直轴 → **偏航**
        hip_y → 绕骨盆左右轴 → **俯仰（前后摆腿）**
        knee_y / ankle_y → **俯仰**
    也就是说 **Poppy 没有踝关节的侧摆和偏航** —— 横向是完全欠驱动的。
    这一条决定了两个设计：站立要容忍 30°~45° 的横向摆动；行走时 lin_vel_y
    的指令范围必须给得很小（它没法真正侧移，只能微调）。
"""
...
class PoppySceneCfg(InteractiveSceneCfg):
    ...
    #   track_air_time=True —— 记录抬脚/落脚的空中时间，行走的步态奖励要用
...
##
# 速度指令（基类版本：全为 0，即"保持静止"）
##


@configclass
class CommandsCfg:
    """速度指令基类。全 0 → 任务退化成"保持静止且不出位移"。

    这就是站立任务用的那一份。行走任务把它换成带采样范围的版本
    （见 `walking/walking_env_cfg.py` 的 `WalkCommandsCfg`）——
    这也说明站立和行走本质是**同一个 MDP**，只是指令分布不同。
    """
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        # 速度指令：站立恒为 0，但保留它 → 行走打开 ranges 就能直接用
...
@configclass
class RewardsCfg:
    """站和走共有的奖励项 —— 本质是"像个人样"的约束。

    走路任务在此基础上再加步态项（`WalkRewardsCfg`），但这一份不动。
    每一项都写清"为什么存在"和"去掉会怎样"。
    """

    # ---------------- 任务项 ----------------
    # 站立时指令恒为 0 → 这一项等价于"不许平移"；行走时它才是真正的主任务项。
    ...
    # 反而更好平衡，于是奖励更高 → 学会蹲着不动。这是直立任务最经典的作弊解。
    # 行走任务要把它改成屈膝姿态对应的骨盆高度（见 walking/walking_env_cfg.py）。
    base_height = RewTerm(
...
class EventCfg:
    ...
    # ---- 随机推力 ----
    # 站立任务的验收标准之一是"能抗住随机推力"，所以推力从一开始就要有。
...
@configclass
class PoppyBaseEnvCfg(ManagerBasedRLEnvCfg):
    """站/走共享的环境基类：只定场景、频率、传感器，不定任务目标。

    子类要做的事情只有三件：
      1. 换掉 `scene.robot`（默认姿态不同：站立是零位，行走是轻微屈膝）
      2. 换掉 `commands`（站立恒 0，行走采样）
      3. 换掉 `rewards`（行走加步态项）
    """
