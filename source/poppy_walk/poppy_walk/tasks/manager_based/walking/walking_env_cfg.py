...
| **feet_air_time**      | **+1.0** | **最重要的步态项**：不给的话策略会发明"滑步"——两脚一直贴地、靠摩擦推着走。它奖励"单脚支撑 + 另一脚在空中待够时间" |
| **feet_slide**         | **−0.1** | 脚在地上滑（与 air_time 互补：一个管"抬起来"，一个管"落地后别蹭"） |
...
class WalkRewardsCfg(RewardsCfg):
    ...
    # ---- 步态项 ----
    # 抬脚时间：奖励"单脚支撑、另一脚在空中待够 0.3 s"。
    # 阈值取 0.3 s 的依据：Poppy 腿长 0.386 m、目标速度 0.35 m/s，
    # 一个完整步态周期约 0.6~0.8 s，单脚腾空时间就是 0.3~0.4 s 量级。
    # 阈值给得比实际能力略高一点是刻意的：它把"抬得高一点、待得久一点"变成收益，
    # 推着策略离开"两脚贴地滑行"的局部解。
